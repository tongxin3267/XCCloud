...
function layuiXtree(options) {
    ...
    // var parm = "token=1&signkey=1f626576304bf5d95b72ece2222e42c3";
    // "merchId=B4201030001&token=1&signkey=1f626576304bf5d95b72ece2222e42c3";
    ...
    obj.onreadystatechange = function () {
        if (obj.readyState == 4 && obj.status == 200 || obj.status == 304) { //回调成功
            ...
            _this._dataJson=_this._dataJson.result_data;
            console.log(_this._dataJson);
...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked+xtree_isdisabled+ '' +
                ' value="' + d[i].functionId + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck">';
