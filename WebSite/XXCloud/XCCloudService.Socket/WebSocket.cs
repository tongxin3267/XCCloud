...
namespace XCCloudService.SocketService.TCP
{
    public class Server
    {
                ...
                if (length > 0)
                {
                    ...
                    if (TCPClientHelper.IsConnected(SessionPool, IP))
                    {
                        ...
                        if (msg != "")
                        {
                            ...
                            Thread commandProcessThread = new Thread(new ParameterizedThreadStart(CommandProcess)) { IsBackground = true, Name = "在线检查线程" };
        ...
        private void CloseSocketClient(Socket SocketClient,string IP)
        {
            try
            {
        ...
        {
            int messageType = 0;
            CommandProcessModel cpModel = (CommandProcessModel)(obj);
            Socket socketClient = cpModel.socketClient;
            string msg = cpModel.msg;
            string IP = cpModel.IP;
            try
            {
            ...
            switch (messageType)
            {
                case (int)(TCPMessageType.会员客户端注册): CommandProcessHandler.Register(socketClient, msg, SessionPool, IP); break;
                case (int)(TCPMessageType.推送消息): CommandProcessHandler.Push(socketClient, msg, SessionPool, IP); break;
                case (int)(TCPMessageType.雷达操作回复): CommandProcessHandler.RadarAnswer(socketClient, msg, SessionPool, IP); break;
            }
        }
    ...
    public class CommandProcessModel
    {
        public CommandProcessModel(Socket socketClient, string msg, string IP)
        {
