...
namespace XXCloudService.RadarServer
{
    ...
    public class UDPServerHelper
    {
        ...
        public string ListenDeviceAddress;
        public string ListenRouterToken;
        ...
        public delegate void 状态更新(Recv机头网络状态报告 cmd);
        public event 状态更新 OnChangeState;
        protected void ChangeState(Recv机头网络状态报告 cmd)
        {
        ...
        public void SendData(byte[] data, EndPoint point)
        {
            try
            {
                ...
                if (PubLib.SendDataLength >= 1000000000)
                {
                    //重置 防止溢出
                    PubLib.SendDataLength = 0;
                }
                PubLib.SendDataLength += data.Length;
        ...
        public static Dictionary<string, RouteInfo> RouteList = new Dictionary<string, RouteInfo>();
